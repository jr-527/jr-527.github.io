// Initializes and returns a presentation wrapper object.
function PptWrapper() {
	...
	let initializePpt = function() {
		...
		ppt.defineSlideMaster({
			...
			objects: [{ image: { data: stellaLogo, w: 192/96, h: 73/96, 
				//x: WIDTH-192/96, y: 0, 
	...
	this.addImageToSlide = function(dataURL, width, height, imageNotes='') {
		if (this.slides[this.currentSlide].length >= maxImagesPerSlide) {
			alert("Limit of " + maxImagesPerSlide + " images per slide! This image was not added.");
	...
	this.removeImageFromSlide = function(n=-1) {
		let slide = this.slides[currentSlide];
		if (slide.length == 0) {
			return;
		}
		if (n < 0) {
		...
		if (n < this.slides[currentSlide].length) {
			slide.splice(n, 1);
		}
	}
	...
	this.exportPresentation = function(nameOfFile='Stella images.pptx') {
		for (i in this.slides) {
			...
			for (j in slideArray) {
				...
				let imgW = Math.min(this.slideWidth/n, this.slideHeight * picW/picH);
				let imgH = Math.min(this.slideHeight, this.slideWidth/n * picH/picW);
				newSlide.addImage({ data: b64,
					w: picW, h: picH,
					x: i*this.slideWidth/n + (this.slideWidth/n - imgW)/2,
					y: (this.slideHeight - imgH)/2,
				...
				if (slideArray[j].notes.length > 0) {
					...
					*/
					let x = parseInt(j)+1;
					slideNotes += 'Image ' + x + ': ' + slideArray.notes + '\n';
				}
			}
